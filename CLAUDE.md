...
分析完成后，对 Wilson Score 低置信的实体做定向数据增强：

1. **自动筛选目标**：从分析结果中筛出 `neg=0 且 pos >= 5` 的实体（有一定好评量但零差评，统计上不可信）
2. **生成定向搜索词**：为每个实体生成多组搜索词，例如：
   - `"北广济街 踩雷"` / `"北广济街 避雷"` / `"北广济街 难吃"`
   - `"贾三 差评"` / `"贾三 不推荐"`
3. **专项搜索**：用这些词在 Bili + XHS 上做专项搜索，补充差评样本
4. **重新计算**：合并新数据后重新运行 `build_report.py`，更新 NSS / Wilson Score

**目的**：零差评大概率是样本不足而非真无差评，定向补采后 Wilson Score 才有意义。
